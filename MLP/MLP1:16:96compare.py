...
def train_and_evaluate(seq_len):
    ...
    return mae, rmse, nmae, nrmse, r2W

def main():
    print(f"单站 {STATION} 不同输入长度 MLP 对比 (输入归一化 + 原始功率输出)")
    seq_lengths = [1, 16, 96]
    results = []
    for L in seq_lengths:
        metrics = train_and_evaluate(L)
        results.append({'seq_len': L, 'MAE': metrics[0], 'RMSE': metrics[1],
                        'NMAE': metrics[2], 'NRMSE': metrics[3], 'R2': metrics[4]})
    print("\n" + "="*80)
    print("汇总对比 (测试集):")
