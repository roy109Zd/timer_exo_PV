...
class DirectMLP(nn.Module):
    def __init__(self, in_features=FEAT_DIM, hidden_dim=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, 1)          # 直接输出预测值
        )
    def forward(self, x):
        # x shape: (batch, window, in_features)
        # 对每个时间步独立进行全连接运算
        out = self.fc(x)                      # (batch, window, 1)
        return out
...
def train_and_evaluate(X_train_sub, y_train_sub, timer_train_sub,
    ...
    # 重新初始化模型（直接输出预测值的MLP）
    model = DirectMLP(in_features=FEAT_DIM, hidden_dim=16).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for Xb, yb, tb in train_loader:
            optimizer.zero_grad()
            pred = model(Xb)                     # 直接输出预测值，shape: (batch, window, 1)
    ...
    model.eval()
    with torch.no_grad():
        pred_test = model(X_test_t).cpu().numpy().squeeze(-1)   # (batch, window)

    y_test_np = y_test_t.cpu().numpy()
    flat_mlp = pred_test.flatten()
    ...
    mlp_mae, mlp_rmse, mlp_nmae, mlp_nrmse, mlp_r2 = compute_metrics(flat_true, flat_mlp, f"直接MLP({ratio_name})")

    return {
        'ratio': ratio_name,
        'timer_mae': timer_mae, 'timer_rmse': timer_rmse, 'timer_r2': timer_r2,
        'affine_mae': mlp_mae, 'affine_rmse': mlp_rmse, 'affine_r2': mlp_r2   # 字段名保持原样，实际值为MLP的结果
    }

# ==================== 主程序（随机打乱子集） ====================
def main():
    print(f"单站 {STATION} | 直接输出预测值的 MLP（替代条件仿射） | 窗口={WINDOW}")
    ...
    print("="*80)
    print(f"{'比例':<8} | {'Timer MAE':>10} | {'MLP MAE':>10} | {'Timer RMSE':>10} | {'MLP RMSE':>10} | {'Timer R2':>8} | {'MLP R2':>8}")
    print("-"*80)
    for r in results:
